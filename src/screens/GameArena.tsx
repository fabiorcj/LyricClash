...
export default function GameArena() {
  ...
  const safeSongId = songId || "_dK2tDK9grQ";
  const songData = (songsData as any)[safeSongId];
  ...
  const getProcessedWords = () => {
    const rawText = songLyrics[currentLineIndex].text;
    ...
    const numGaps = Math.max(1, Math.min(words.length, Math.floor(words.length * gapRatio)));
        ...
        <div className="absolute top-4 right-4 sm:top-6 sm:right-8 z-30 flex flex-row sm:flex-col gap-2 sm:gap-3 max-w-[95%] sm:max-w-xs overflow-x-auto no-scrollbar pointer-events-none">
          {sortedPlayers.slice(0, 5).map((p: any, idx: number) => {
            ...
            return (
              ...
              >
                <div className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full shadow-[0_0_8px_currentColor] ${idx === 0 ? "bg-yellow-400 text-yellow-400" : isMe ? "bg-emerald-400 text-emerald-400" : "bg-blue-400 text-blue-400"}`} />
                <div className="flex flex-col min-w-0">
                  ...
                  <span className="text-[10px] sm:text-[12px] font-black text-white -mt-0.5">
                    {p.score} <span className="text-[7px] sm:text-[8px] opacity-30">XP</span>
                  </span>
                </div>
                {p.combo >= 2 && <span className="text-xs sm:text-sm animate-pulse">🔥</span>}
                
                {isHost && isMe && (
                  <button
                    onClick={() => socketManager.socket?.emit("force_return_lobby", { roomId })}
        ...
        <div className="absolute top-4 sm:top-6 w-full flex flex-col items-center sm:items-start justify-start px-6 sm:px-10 gap-2">
          <div className="flex flex-col sm:flex-row justify-between w-full gap-2">
            ...
            <div className="px-3 sm:px-4 py-1 sm:py-1.5 rounded-full glass-panel flex items-center gap-1.5 w-fit text-[7px] sm:text-[9px] text-gray-500 font-bold uppercase tracking-[0.1em]">
              Mínimo:{" "}
              <span className="text-blue-400 font-black">{roomDifficulty === 'easy' ? '30%' : roomDifficulty === 'medium' ? '50%' : roomDifficulty === 'hard' ? '70%' : '90%'}</span>
            </div>
          </div>
        </div>
        ...
        {gameState === "idle" || gameState === "playing" ? (
          ...
          >
            ...
            <button
              ...
              className="px-16 py-5 border border-blue-500/50 bg-blue-600/20 hover:bg-blue-500 hover:text-black rounded-2xl font-black text-blue-400 shadow-[0_0_30px_rgba(59,130,246,0.3)] hover:scale-105 active:scale-95 transition-all text-sm tracking-[0.3em] uppercase"
            >
