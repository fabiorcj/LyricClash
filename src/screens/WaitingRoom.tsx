...
export default function WaitingRoom() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      socketManager.joinRoom(roomId || 'GLOBAL', username || 'Convidado', difficulty);
      ...
      socketManager.socket?.on('room_state_update', (roomData) => {
        ...
        const me = Object.values(roomData.players).find((p: any) => p.username === username) as any;
        if (me && me.isHost) {
          setIsHost(true);
        }
      ...
      socketManager.socket?.on('game_starting', ({ songId }) => {
         navigate(`/arena/${roomId}/${encodeURIComponent(username || '')}/${songId}`);
      });
  ...
  const handleStartGame = () => {
    if (isHost) {
       socketManager.socket?.emit('start_game_request', { roomId, songId: selectedSong });
    }
  };
      ...
      >
        ...
        <div className="flex-1 flex flex-col h-auto md:h-full overflow-hidden">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-6 mb-6 flex flex-col items-center sm:items-start group hover:border-emerald-500/30 transition-all duration-500 shadow-inner">
            <h2 className="text-gray-500 font-bold tracking-[0.3em] text-[8px] sm:text-[10px] uppercase mb-1">Código da Sala</h2>
            ...
            >
               <span className="group-hover/btn:-translate-x-1 transition-transform">←</span> Abandonar
            </button>
          </div>
          
          <h3 className="text-gray-500 font-bold tracking-widest text-[10px] uppercase mb-4 border-b border-white/5 pb-2">Competidores na Escuta ({players.length})</h3>
          
          <div className="flex-1 flex flex-col gap-3 min-h-0 overflow-y-auto pr-2 custom-scrollbar max-h-[150px] sm:max-h-none">
            {players.length === 0 && <span className="text-gray-600 text-sm">Carregando lista...</span>}
            {players.map((p: any, i) => (
              <motion.div 
              ...
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg ${p.isHost ? 'bg-gradient-to-tr from-yellow-400 to-orange-500 shadow-[0_0_15px_rgba(250,204,21,0.4)] ring-2 ring-yellow-300/50' : 'bg-gray-800'}`}>
                  {p.isHost ? '👑' : '👾'}
                </div>
                <div className="flex-1">
                  <p className="font-bold text-gray-200">{p.username} {p.username === username ? '(Você)' : ''}</p>
                  <p className={`text-[10px] uppercase tracking-widest font-black ${p.isHost ? 'text-yellow-400' : 'text-gray-500'}`}>{p.isHost ? 'Dono da Sala / Host' : 'Convidado'}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 h-[320px] sm:h-full bg-black/40 rounded-[30px] p-6 sm:p-8 border border-white/5 shadow-inner relative overflow-hidden">
          ...
          <h3 className="text-gray-400 font-bold tracking-[0.3em] text-xs uppercase mb-6 z-10 text-center">Setlist da Batalha</h3>
              ...
              {Object.entries(songsData).map(([id, song]: [string, any]) => (
                <button
                  ...
                  onClick={() => {
                    setSelectedSong(id);
                    if (isHost) {
                      socketManager.socket?.emit('change_song', { roomId, songId: id });
                    }
                  }}
                ...
                >
                  ...
                  <div className="flex justify-between items-center relative z-20">
                    <div className="min-w-0 flex-1 pl-2">
                      <div className="font-black text-base sm:text-lg text-white mb-0.5 truncate drop-shadow-md">{song.title}</div>
                      <div className="font-bold text-gray-500 text-[10px] sm:text-xs tracking-widest uppercase truncate">{song.artist}</div>
                    </div>
                    {selectedSong === id && (
